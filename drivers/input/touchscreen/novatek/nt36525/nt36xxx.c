...
static int pinctrl_configure(struct nvt_ts_data *ts, bool enable)
{
	...
	if (!IS_ERR_OR_NULL(state))
		return pinctrl_select_state(ts->platdata->pinctrl, state);
...
//		ret = ERR_PTR(-ENXIO);
	...
	pr_info("zone's size - indicator:%d, navigation:%d, edge:%d\n",
		platdata->area_indicator, platdata->area_navigation, platdata->area_edge);

	pr_info("irq_flags: 0x%X, resolution: (%d, %d), max_touch_num: %d\n",
		platdata->irq_flags,// platdata->firmware_name,
		platdata->abs_x_max, platdata->abs_y_max,
		platdata->max_touch_num);
		
	platdata->support_ear_detect = of_property_read_bool(np, "support_ear_detect_mode");
	pr_info("%s: ED:%d\n", __func__, platdata->support_ear_detect);
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	if (ts->platdata->support_ear_detect && ts->power_status == LP_MODE_STATUS) {
...
*/
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	if(ts->power_status == LP_MODE_STATUS && ts->prox_power_off) {
		input_info(true, &ts->client->dev,"%s lpm && prox_power_off\n", __func__);
		mutex_unlock(&ts->lock);
		return IRQ_HANDLED;
	}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	input_info(true, &client->dev, "%s: lcdtype 0x%06X\n", __func__, lcdtype);
	...
	if (lcdtype == 0x8AD221) {
		input_info(true, &client->dev, "%s: PANEL is BOE\n", __func__);
		return -EINVAL;
	}

	if (lcdtype == 0x8AA240) {
		input_info(true, &client->dev, "%s: PANEL is CSOT\n", __func__);
	}
	...
	if (!platdata) {
		...
		if (IS_ERR(platdata)) {
			input_err(true, &client->dev,"parse dt failed!\n");
...
/*
	...
	*/
	...
	if (ts->input_dev == NULL) {
		pr_err("allocate input proximity device failed\n");
	...
	if (client->irq) {
		input_err(true, &client->dev,"int_trigger_type=%d\n", ts->int_trigger_type);
		ts->irq_enabled = true;
...
{
	input_info(true, &client->dev,"Removing driver...\n");
...
{
	input_info(true, &client->dev,"Shutdown driver...\n");
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if (ts->power_status == POWER_OFF_STATUS) {
		input_info(true, &ts->client->dev,"Touch is already suspend\n");
		return 0;
	}
